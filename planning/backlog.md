...
- [ ] **The Week 4 deck's PPTX is Marp's image-per-slide export**, so its text is not editable in
      PowerPoint. If an editable deck is wanted, the `--pptx-editable` route needs LibreOffice.
- [ ] **QGIS screenshots show QGIS 3.38 on Windows; the download page now offers LTR 3.44 and
      4.2.** Menus and dialogs are the same, but rerun `fig_tools/qgis_shots/` after the fleet
      laptops upgrade so the pictures match what students see.
...
- 2026-09-22 — **Week 4 QGIS lecture deck** (`docs/slides/week_04.md`, 26 slides, exported to HTML,
  ...
  `docs/software/images/qgis_NN_*.{jpg,png}`, generated by scripting QGIS itself from the lab
  orthophoto (`fig_tools/qgis_shots/`; the orthophoto is git-ignored in `.claude/local/`). A public
  **Slides** page, `class_resources/lecture_slides.md`, is in the nav after Start Here and under
  Reference, listing every deck; `instructors/slides.md` now says to add a row there.
