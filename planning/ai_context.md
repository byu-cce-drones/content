...
**Last done (2026-09-08).** A public **Instructors** section at the bottom of the nav
(`docs/instructors/`: index, editing, figures, slides) for TAs and future instructors, condensed from
`planning/instructions.md`; and the **Marp lecture-slide toolchain**: `docs/slides/theme.css` (house
palette), `docs/slides/week_05.md` as the template deck (19 slides built from the Measurement
Fundamentals figures), exports `week_05.html`/`.pdf` committed next to it and linked from
`weeks/week_05.md`, `fig_tools/build_slides.py` to rebuild every deck, and `exclude_docs: slides/*.md`
in `mkdocs.yml` so sources are not pages. Setup and process are on `instructors/slides.md`. No
session currently owns any files; concurrent sessions need only the rules below.

**Last done (2026-09-22).** The **Week 4 QGIS lecture deck**, `docs/slides/week_04.md`, with
seventeen real QGIS screenshots in `docs/software/images/` generated by a PyQGIS capture script in
`fig_tools/qgis_shots/` (QGIS is installed at `C:\OSGeo4W`; the Lab 2 orthophoto it needs is
git-ignored in `.claude/local/`). Exports are HTML, PDF and PPTX. A **Slides** page
(`class_resources/lecture_slides.md`) sits in the nav after Start Here and under Reference. Note
that the nav now groups Weeks 10–15 under one "Final Project Planning" section and the
